      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <Card className="lg:col-span-2 p-6">
          ...
          <p className="mt-1 text-sm text-ink-700">
            Atur aktivitas & tujuan. Data profil (jenis kelamin, usia, tinggi, berat) di halaman{" "}
          ...
          {!isProfileReady(profile) && (
            <div className="mb-4 rounded-xl border border-line-200 bg-surface-100 p-3 text-sm text-ink-700">
              ...
              <Link to="/profile" className="underline">halaman Profile</Link>.
            </div>
          )}

          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label htmlFor="activity" className="text-sm font-medium text-ink-900">Aktivitas</label>
              <select
                id="activity"
            ...
            <div>
              <label htmlFor="goal" className="text-sm font-medium text-ink-900">Tujuan</label>
              <select
                id="goal"
                className="mt-1 w-full rounded-xl border border-line-200 bg-surface-100 px-3 py-2 text-sm focus:outline-none focus:ring-4 focus:ring-brand-100 focus:border-brand-500 text-ink-900"
                value={settings.goal}
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3 text-sm">
            <div className="rounded-xl border border-line-200 bg-surface-100 p-3">
              ...
              <p className="text-lg font-semibold text-ink-900">{bmr.toLocaleString()} kcal</p>
            </div>
            <div className="rounded-xl border border-line-200 bg-surface-100 p-3">
              <p className="text-ink-700">TDEE (maintain)</p>
              <p className="text-lg font-semibold text-ink-900">{tdee.toLocaleString()} kcal</p>
            </div>
            <div className="rounded-xl border border-line-200 bg-surface-100 p-3">
        ...
        <Card className="p-6 flex flex-col justify-center">
          ...
          <p className="mt-2 text-sm text-ink-700">
            Target harian <span className="font-semibold text-ink-900">{target.toLocaleString()} kcal</span>.<br />{focus}
          </p>
          <p className="mt-3 text-xs text-ink-700">* Perkiraan umum, bukan nasihat medis.</p>
        </Card>
      </div>

      {/* === Satu-satunya summary di home === */}
      <div className="mt-6">
